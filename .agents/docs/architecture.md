...
File Fact IR (SfcFacts / ScriptFacts / TemplateFacts)  — stable, rule-facing
        ...
        still runs the ordinary source collector, watch-family option/signature
        facts, and callback-contract collectors (`watch_callbacks.rs`). Combined
        `RULESET_VERSION` is 20; `REACTIVITY_GRAPH_VERSION` stays 41. Lifetime facts are a
