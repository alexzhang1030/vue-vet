...
**Session locality.** `DirtyPlan.export_closure` is the last A6 seed-dirty set (`TraceModulesReport::seed_plan_dirty`), not a clone of `module_summaries`. The linker still computes the set. After a warm persist scan the pipeline borrows source-dirty `ModuleSource`s and sets `retain_cached_modules` (deleted ids in `drop_module_ids`). It does not clone the live universe into `live_module_ids` or into the tracer input. The tracer merges cached summaries into linking, pulls seed-dirty consumers from `cached_source` (`Arc<ModuleSource>`), and leaves unchanged graphs in `state`. The report is this-pass only. Layers read `cached_reactivity` and reuse the layered `Arc<[ModuleReactivity]>` when the key matches. `module_summaries_visited` is phase-one successes+failures (the dirty subset), not `graph.module_reactivity.len()`. One-shot `trace_*` benches force `persist_linking_cache = false` and are mostly unseeded. `trace_warm_leaf_edit_1k_modules` is the CodSpeed name that keeps `persist_linking_cache` on, sets retain, and edits one independent leaf without cloning the other N-1 sources.
...
10. ~~Make `DirtyPlan.export_closure` the linker dirty set.~~ Session reports `seed_plan_dirty` as `export_closure`. Pipeline borrows source-dirty modules and sets `retain_cached_modules`; the tracer merges cached summaries and pulls seed-dirty consumers. The linker still computes the closure — there is no second algorithm. Dual-path: subset state graphs ≡ full-list state graphs. The report is this-pass only.
...
**The interesting remaining speed class** is nested helper hops (they still rediscover callees; do not cache those without a visiting-set story), not session input breadth. Warm scans borrow a source-dirty subset and retain the cached universe without cloning modules. The warm leaf-edit CodSpeed name is the locality signal; do not treat `trace_1k_modules` as that win. Do not invent a second export-closure algorithm.
