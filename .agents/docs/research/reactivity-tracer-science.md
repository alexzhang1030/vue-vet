...
Cold cost is dominated by (1) visiting every module in phase one, (2) cloning maps on the export worklist, (3) full Oxc reparse of every module whose seed plan can materialize, (4) budgeted external/companion I/O. Incremental reuse *inside* `ModuleTraceState` is real. Warm persist scans pass a source-dirty subset, not the full module set.

**Single-file walks.** The file builds `FileTraceIndex` once (`LocalCalleeIndex` `full_callees(F)` plus `ScopeNodeIndex`) and `ScopeIrIndex` once (await ends + pause IR). Classify looks up. Await ownership is a third rule: first Function/Arrow, unless `if` / ternary / `&&` sits in between. Do not unify it with context/sync — there is no HOF / `toValue` / deferred skip. Pause stays `innermost_function_id` plus helper-exit leak; last-event fold, not a stack. Do not cache a slice keyed only on `scope_id`. Nested hops filter `visiting` on the callee map. Context vs sync maps stay separate (deferred is outside vs drop).

**Phase two is all-or-nothing for a module that can materialize.** Known / ValueBag / ComponentFactory / inject plans still reparse the whole file. Factory / Composable / ValueFactory / callback-slot plans reparse only when phase-one `called_locals` contains the name. Empty plans and unused call-site-only plans reuse `local_graph`. `called_locals` is not a linking-surface field — do not filter `ModuleSeedPlan` by call sites (that would miss the linking cache on a body edit). Do not invent a second seed-apply.

**Session locality.** `DirtyPlan.export_closure` is the last A6 seed-dirty set (`TraceModulesReport::seed_plan_dirty`), not a clone of `module_summaries`. The linker still computes the set. After a warm persist scan the pipeline passes source-dirty `Arc<ModuleSource>`s via `trace_modules_incremental_from_arcs` and sets `retain_cached_modules` (deleted ids in `drop_module_ids`). It does not clone the live universe into `live_module_ids` or into the tracer input. Persist of a dirty input is that Arc, not `ModuleSource::clone`. The tracer compares live surfaces in place from `state.entries` and merges cached summaries into this-pass facts only on a linking miss, then pulls seed-dirty consumers from `cached_source` and leaves unchanged graphs in `state`. The report is this-pass only. Layers read `cached_reactivity`, store `Arc<[Arc<ModuleReactivity>]>`, and patch the layered key in place on a leaf edit. Structural indexes are skipped when every file reuses. `module_summaries_visited` is phase-one successes+failures (the dirty subset), not `graph.module_reactivity.len()`. One-shot `trace_*` benches force `persist_linking_cache = false` and are mostly unseeded. `trace_warm_leaf_edit_1k_modules` is the CodSpeed name that keeps `persist_linking_cache` on, sets retain, and edits one independent leaf without cloning the other N-1 sources.
...
ECOOP 2025 *Reusing Caches and Invariants* (function-summary reuse when the analysis context is unchanged) maps to `ModuleSeedPlan` equality, which already exists. Phase one already visits only the dirty leaf. A linking-cache hit compares live surfaces in place and skips merging N-1 cached summaries into this-pass facts (`cached_modules_merged == 0`). Do not invent a second summary IR. The remaining speed class is whole-file reparse for plans that can materialize.
...
10. ~~Make `DirtyPlan.export_closure` the linker dirty set.~~ Session reports `seed_plan_dirty` as `export_closure`. Pipeline passes source-dirty `Arc<ModuleSource>`s (`from_arcs`) and sets `retain_cached_modules`; persist is a refcount. The tracer compares live surfaces in place and merges cached summaries only on a linking miss, then pulls seed-dirty consumers. The linker still computes the closure — there is no second algorithm. Dual-path: subset state graphs ≡ full-list state graphs. The report is this-pass only. Layers keep `Arc<[Arc<ModuleReactivity>]>` and patch the key in place on a leaf edit. `cached_modules_merged` is 0 on a leaf-edit hit and N-1 on a producer-export miss.
11. ~~Compute `local_zero_arg_callees_in_scope` once per `finish_scope` and pass the set into reads / uncertain / writes.~~ `FileTraceIndex` (callee + local member/ident/call/write buckets) plus `ScopeIrIndex` (await ends + pause IR once per file). Nested hops filter `visiting`. Context vs sync maps stay separate. Await is a third owner map. Phase two skips unused call-site-only plans via `called_locals`. Leftover in this row is whole-file reparse for plans that *can* materialize (Oxc `Semantic` is not `Send`; no second apply). No graph bump (same facts). `trace_1k_modules` is mostly unseeded `ref()` modules — no-regression, not the win signal.
...
**The interesting remaining speed class** is whole-file reparse for plans that can materialize (Known / used Factory / inject), not unused call-site-only plans, not helper-callee rediscovery, not local member/ident/write/uncertain walks, not await/pause IR rebuilds, not session input breadth, and not a linking-hit merge of every attached summary. Nested hops filter `visiting` on the file callee index. Local collectors look up `ScopeNodeIndex`. Classify looks up `ScopeIrIndex`. Warm scans pass a source-dirty `Arc<ModuleSource>` subset and retain the cached universe without cloning modules. A linking-cache hit compares live surfaces in place (`cached_modules_merged == 0`). Layers share `Arc<ModuleReactivity>` per unchanged leaf. The warm leaf-edit CodSpeed name is the locality signal; do not treat `trace_1k_modules` as that win. Do not invent a second export-closure algorithm. Do not invent a second seed-apply path to avoid the reparse. Do not put `called_locals` on the linking surface. Do not skip merge on a linking miss — `resolve_exports` and consumer pull still need the full live `facts_by_id`.
