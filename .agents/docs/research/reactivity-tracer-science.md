...
**Single-file walks.** Each tracking scope still walks all nodes for await/pause IR. The file builds `FileTraceIndex` once: `LocalCalleeIndex` (`full_callees(F)`) plus `ScopeNodeIndex` (members / idents / calls via `scope_context`; writes via `sync_tracking_owns_node`). Do not unify those two maps — deferred callbacks stay in context (outside) and drop from sync. Nested hops filter `visiting` on the callee map. Do not cache a slice keyed only on `scope_id`.
...
11. ~~Compute `local_zero_arg_callees_in_scope` once per `finish_scope` and pass the set into reads / uncertain / writes.~~ `FileTraceIndex`: callee map plus local member/ident/call/write buckets. Nested hops filter `visiting`. Context vs sync maps stay separate. Await/pause IR still walk. No graph bump (same facts). `trace_1k_modules` is mostly unseeded `ref()` modules — no-regression, not the win signal.
...
**The interesting remaining speed class** is await/pause IR walks and phase-two all-or-nothing reparse, not helper-callee rediscovery, not local member/ident/write/uncertain walks, and not session input breadth. Nested hops filter `visiting` on the file callee index. Local collectors look up `ScopeNodeIndex`. Warm scans pass a source-dirty `Arc<ModuleSource>` subset and retain the cached universe without cloning modules. Layers share `Arc<ModuleReactivity>` per unchanged leaf. The warm leaf-edit CodSpeed name is the locality signal; do not treat `trace_1k_modules` as that win. Do not invent a second export-closure algorithm.
