...
| 2026-08-27 | Subset tracer input | Warm persist scans borrow source-dirty `ModuleSource`s and set `retain_cached_modules` (drop deleted ids only). Report is this-pass; unchanged graphs stay in `ModuleTraceState`. `ProjectGraph.module_reactivity` is `Arc<[ModuleReactivity]>` so a no-op rescan shares the layered slice. No graph version bump. |
