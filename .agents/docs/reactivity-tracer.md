...
| 2026-08-27 | Subset tracer input | Warm persist scans pass source-dirty `ModuleSource`s plus `live_module_ids`. Pipeline does not clone unchanged sources to build that list. Tracer merges cached summaries, pulls seed-dirty consumers, emits the live universe. No graph version bump. |
