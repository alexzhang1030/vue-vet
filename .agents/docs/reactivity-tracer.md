...
| 2026-08-27 | Subset tracer input | Warm persist scans pass source-dirty `ModuleSource`s plus `live_module_ids`. Pipeline does not clone unchanged sources. Cache holds `Arc<ModuleSource>`. Layers reuse unchanged module Arcs (no `make_mut` of the other N-1 graphs). No graph version bump. |
