...
The workspace tracks the latest stable Rust release and latest stable edition, following Rolldown's toolchain baseline. The repository pins the exact compiler in `rust-toolchain.toml`; all crates inherit the workspace `rust-version` and edition. Shipped CLI/npm binaries use `profile.release` (`opt-level = 2`, `lto = "fat"`, `panic = "abort"`, `strip = "symbols"`) plus `opt-level = "z"` on runtime protocol/UI packages (`tower-lsp`, `lsp-types`, `vue_vet_lsp`, `vue_vet_mcp`, `clap`, `clap_builder`, `ratatui`, `ratatui-core`, `ratatui-widgets`, `ratatui-crossterm`, `crossterm`) and on measured product crates (`vue_vet_rules`, `vue_vet_practice`, `vue_vet_rule_query` at `opt-level = "z"`; `vue_vet_core`, `vue_vet_reactivity`, `vue_vet_session`, `vue_vet_project`, and `vue_vet_cache` at `opt-level = "s"`). The LSP/TUI-only runtime closure is also size-optimized (issue #241): `tokio` at `opt-level = "s"`, and `tokio-util`, `tower*`, `futures*`, `httparse`, `bytes`, `pin-project`, `slab`, the `url` / `idna` / `icu_*` / `zerovec` family that only `lsp-types` pulls in, and `mio` / `signal-hook*` that only `crossterm` pulls in at `opt-level = "z"`; `cargo tree --invert` proved none of them is reachable from the CLI scan path or from any Divan bench closure, so the acceptance gate for that group is a real LSP workflow (initialize, open, versioned diagnostics, unsaved change, two hovers, shutdown; 16 samples per side, ABBA, 5% median) plus byte-identical LSP/MCP protocol output, not the analysis benches. Parser, `vue_vet_oxc`, resolver (including `pnp` / `regress` / `percent-encoding`), regex, serde, and `vue_vet_reporters` stay on the profile default. Proc-macros are not overridden. Linux ARM64 file bytes move in 64 KiB steps: the ELF segments are aligned to the 64 KiB max page size, so a text reduction only changes the file size when it crosses a page boundary (the `vue_vet_cache = "s"` and #241 closures each removed exactly one page); measure the actual Linux ARM64 artifact, do not extrapolate from macOS bytes. Fat LTO still inlines across crates. Absolute file bytes vary by target; each platform gate uses its own artifact. `just native-size` builds then measures via Cargo JSON; CI measures the already-built matrix artifact with `scripts/native_size.py --binary --target --budget-file` and `fixtures/quality/native-size-budget.json` (gzip-9 is a binary compression proxy, not npm/archive bytes). Maxima are ceil(candidate bytes × 1.03) from pkg.pr.new run 34039577281 (`fa2debc`); baseline sizes are run 34034720314 (`2dabaad`). Those product-crate overrides are accepted only with exact CLI and cache output equality plus five separate same-tree median 5% gates: 5k no-cache CLI, 5k fresh-cache CLI (serialization, write, rename, then an observed hit), and `whole_project::scan_cold_mixed_1k`, `whole_project::scan_warm_mixed_1k`, and `whole_project::json_render_mixed_1k` (not `analyze_sfc` alone). See [quality baselines](../../docs/quality-baselines.md). `profile.codspeed` restates `opt-level = 3` for those eight packages so CodSpeed keeps instrumentation `opt-level = 3`. The remaining mass is still one process (Oxc + Vize + product IR + LSP/MCP/TUI). Unused LightningCSS and ratatui termwiz backends stay out of the lock. Do not add UPX or a second analysis binary. Rolldown's lint policy is a floor: Vue Vet additionally denies the Clippy `all`, `cargo`, `pedantic`, and `nursery` groups, forbids unsafe Rust, and denies panic-prone conveniences such as unchecked indexing, string slicing, `unwrap`, and `expect`. The group-level duplicate-version exception covers the reviewed Vize and atomic-writer dependency graphs; each additional duplicate still requires explicit rationale. `just` is the task runner and the canonical interface for local and CI validation. `prek` manages Git hooks from `.pre-commit-config.yaml` without adding a Python runtime requirement.
