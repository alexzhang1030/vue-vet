//! Matrix family rules: shared detection types, unique `RuleMeta` per id.
//!
//! Hand-maintained. `scripts/gen_matrix_rules.py` is a check-only inventory
//! validator and must not overwrite this file.

use std::collections::BTreeMap;

use vue_vet_core::{
  Confidence, FactKinds, FactRef, PRACTICE_CATEGORY, ReactiveBindingKind, ReactiveReadKind, Rule,
  RuleContext, RuleMeta, ScriptBlockFacts, Severity, TrackingScopeFact, TrackingScopeKind,
};

use vue_vet_rule_query::{
  binding_path, canonical_write_identity, effect_family, is_readonly_kind, reactive_binding,
  reactive_binding_for_operand, same_reactive_target, script_block,
  setup_calls_after_first_top_level_await, write_path,
};

struct BoundaryRule {
  meta: &'static RuleMeta,
  scope_kind: TrackingScopeKind,
  read_kind: ReactiveReadKind,
  scope_label: &'static str,
  reason: &'static str,
}

impl Rule for BoundaryRule {
  fn meta(&self) -> &'static RuleMeta {
    self.meta
  }

  fn fact_kinds(&self) -> FactKinds {
    FactKinds::TRACKING_SCOPE
  }

  fn run_on(&self, fact: FactRef<'_>, context: &mut RuleContext<'_>) {
    let FactRef::TrackingScope { scope, .. } = fact else {
      return;
    };
    if scope.kind != self.scope_kind {
      return;
    }
    for read in &scope.reads {
      if read.kind != self.read_kind {
        continue;
      }
      let path = binding_path(read);
      context.report(
        self.meta(),
        read.span,
        format!(
          "`{path}` is read {reason} inside {label}, so it is not a reliable dependency",
          reason = self.reason,
          label = self.scope_label
        ),
        Some(format!(
          "Read `{path}` synchronously in {label}, or use explicit `watch` sources.",
          label = self.scope_label
        )),
      );
    }
  }
}

macro_rules! boundary_rule {
  ($static_name:ident, $id:literal, $doc:literal, $scope:ident, $read:ident, $label:literal, $reason:literal) => {
    static $static_name: BoundaryRule = BoundaryRule {
      meta: &RuleMeta {
        id: $id,
        category: "reactivity",
        default_severity: Severity::Warning,
        confidence: Confidence::High,
        documentation: $doc,
      },
      scope_kind: TrackingScopeKind::$scope,
      read_kind: ReactiveReadKind::$read,
      scope_label: $label,
      reason: $reason,
    };
  };
}

boundary_rule!(
  B_NO_AFTER_AWAIT_DEPENDENCY_IN_COMPUTED,
  "vue-vet/reactivity/no-after-await-dependency-in-computed",
  "rules/reactivity/no-after-await-dependency-in-computed",
  Computed,
  AfterAwait,
  "computed",
  "after `await`"
);
boundary_rule!(
  B_NO_OUTSIDE_TRACKING_DEPENDENCY_IN_COMPUTED,
  "vue-vet/reactivity/no-outside-tracking-dependency-in-computed",
  "rules/reactivity/no-outside-tracking-dependency-in-computed",
  Computed,
  OutsideTracking,
  "computed",
  "outside synchronous tracking"
);
boundary_rule!(
  B_NO_AFTER_AWAIT_DEPENDENCY_IN_WATCH_SOURCES,
  "vue-vet/reactivity/no-after-await-dependency-in-watch-sources",
  "rules/reactivity/no-after-await-dependency-in-watch-sources",
  WatchSources,
  AfterAwait,
  "watch sources",
  "after `await`"
);
boundary_rule!(
  B_NO_OUTSIDE_TRACKING_DEPENDENCY_IN_WATCH_SOURCES,
  "vue-vet/reactivity/no-outside-tracking-dependency-in-watch-sources",
  "rules/reactivity/no-outside-tracking-dependency-in-watch-sources",
  WatchSources,
  OutsideTracking,
  "watch sources",
  "outside synchronous tracking"
);
boundary_rule!(
  B_NO_AFTER_AWAIT_DEPENDENCY_IN_EFFECT_SCOPE,
  "vue-vet/reactivity/no-after-await-dependency-in-effect-scope",
  "rules/reactivity/no-after-await-dependency-in-effect-scope",
  EffectScope,
  AfterAwait,
  "effectScope",
  "after `await`"
);
boundary_rule!(
  B_NO_OUTSIDE_TRACKING_DEPENDENCY_IN_EFFECT_SCOPE,
  "vue-vet/reactivity/no-outside-tracking-dependency-in-effect-scope",
  "rules/reactivity/no-outside-tracking-dependency-in-effect-scope",
  EffectScope,
  OutsideTracking,
  "effectScope",
  "outside synchronous tracking"
);
struct DefineExposeAfterAwaitRule;

impl Rule for DefineExposeAfterAwaitRule {
  fn meta(&self) -> &'static RuleMeta {
    &RuleMeta {
      id: "vue-vet/correctness/no-define-expose-after-await",
      category: "correctness",
      default_severity: Severity::Warning,
      confidence: Confidence::High,
      documentation: "rules/correctness/no-define-expose-after-await",
    }
  }

  fn run_once(&self, context: &mut RuleContext<'_>) {
    for call in setup_calls_after_first_top_level_await(context.script(), "defineExpose") {
      context.report(
        self.meta(),
        call.span,
        "`defineExpose` is registered after a top-level `await`, so it will not bind to this instance"
          .into(),
        Some(
          "Move `defineExpose` before the first top-level `await`, or register it in a synchronous setup path."
            .into(),
        ),
      );
    }
  }
}

static A_DEFINE_EXPOSE: DefineExposeAfterAwaitRule = DefineExposeAfterAwaitRule;
enum PathologyKind {
  SelfTrigger,
  SideEffectsInComputed,
  WriteWithoutRead,
  NoDependency,
  PreferWatchSingle,
  AssignOnlyConditional,
  DisposeRead,
  EmptyWatchSources,
}

struct PathologyRule {
  meta: &'static RuleMeta,
  kind: PathologyKind,
  scope_filter: TrackingScopeKind,
  effect_family_only: bool,
}

impl Rule for PathologyRule {
  fn meta(&self) -> &'static RuleMeta {
    self.meta
  }

  fn fact_kinds(&self) -> FactKinds {
    FactKinds::TRACKING_SCOPE
  }

  fn run_on(&self, fact: FactRef<'_>, context: &mut RuleContext<'_>) {
    let FactRef::TrackingScope { scope, block_kind } = fact else {
      return;
    };
    let matches_scope = if self.effect_family_only {
      effect_family(scope.kind)
    } else {
      scope.kind == self.scope_filter
    };
    if !matches_scope {
      return;
    }
    match self.kind {
      PathologyKind::SelfTrigger => {
        // Computed self-write can invalidate the cached value (cache/purity).
        let Some(block) = script_block(context.script(), block_kind) else {
          return;
        };
        let bindings = &block.reactivity_graph.bindings;
        let primary = primary_writes_by_canonical_target(
          scope.writes.iter().filter(|write| {
            scope.reads.iter().any(|read| {
              read.kind == ReactiveReadKind::Unconditional
                && same_reactive_target(bindings, read, write)
            })
          }),
          bindings,
        );
        for (path, span) in primary.into_values() {
          context.report(
            self.meta(),
            span,
            format!(
              "`{path}` is read and written in this computed getter, so the result is impure and can invalidate its cached value"
            ),
            Some(
              "Keep computed getters pure; move the write to `watch` or an event handler.".into(),
            ),
          );
        }
      }
      PathologyKind::SideEffectsInComputed => {
        let Some(block) = script_block(context.script(), block_kind) else {
          return;
        };
        let primary =
          primary_writes_by_canonical_target(scope.writes.iter(), &block.reactivity_graph.bindings);
        for (path, span) in primary.into_values() {
          context.report(
            self.meta(),
            span,
            format!("computed getter writes `{path}`, which is a side effect"),
            Some("Keep computed getters pure; move writes into `watch` / event handlers.".into()),
          );
        }
      }
      PathologyKind::WriteWithoutRead => {
        if scope.writes.is_empty() || !scope.reads.is_empty() || !scope_coverage_complete(scope) {
          return;
        }
        let (message, help) = absence_finding(
          &format!("`{}` writes reactive state but reads no reactive dependency", scope.callee),
          &format!(
            "`{}` writes reactive state but reads no known reactive dependency",
            scope.callee
          ),
          &scope.uncertain_accesses,
          "Effects without reactive reads never re-run; use a plain function or explicit trigger.",
        );
        context.report(self.meta(), scope.span, message, help);
      }
      PathologyKind::NoDependency => {
        if !scope.reads.is_empty() || !scope_coverage_complete(scope) {
          return;
        }
        let (message, help) = absence_finding(
          "`computed` does not read any reactive dependency",
          "`computed` does not read any known reactive dependency",
          &scope.uncertain_accesses,
          "Return a plain value, or read reactive state inside the getter.",
        );
        context.report(self.meta(), scope.span, message, help);
      }
      PathologyKind::PreferWatchSingle => {
        if !scope.assignment_only {
          return;
        }
        if !scope_coverage_complete(scope) {
          return;
        }
        if scope.reads.len() != 1 {
          return;
        }
        let Some(read) = scope.reads.first() else {
          return;
        };
        if read.kind != ReactiveReadKind::Unconditional {
          return;
        }
        // `watch(source)` with immediate+sync retriggers a self-write, including
        // `const alias = count; alias.value = count.value + 1`.
        let Some(block) = script_block(context.script(), block_kind) else {
          return;
        };
        if scope
          .writes
          .iter()
          .any(|write| same_reactive_target(&block.reactivity_graph.bindings, read, write))
        {
          return;
        }
        let path = binding_path(read);
        let help = prefer_watch_help(scope, block, &path.to_string());
        context.report(
          self.meta(),
          scope.span,
          format!(
            "`{}` only tracks `{path}`; prefer `watch` with an explicit source",
            scope.callee
          ),
          Some(help),
        );
      }
      PathologyKind::AssignOnlyConditional => {
        if !scope.assignment_only {
          return;
        }
        if !scope.reads.iter().any(|read| read.kind == ReactiveReadKind::Conditional) {
          return;
        }
        context.report(
          self.meta(),
          scope.span,
          format!(
            "`{}` is assignment-only but tracks at least one conditional dependency",
            scope.callee
          ),
          Some(
            "Use explicit `watch` sources for every value that must invalidate the sync.".into(),
          ),
        );
      }
      PathologyKind::DisposeRead => {
        for read in &scope.reads {
          let path = binding_path(read);
          context.report(
            self.meta(),
            read.span,
            format!("`{path}` is read in `onScopeDispose`, which is not a tracking scope"),
            Some(
              "Capture values before dispose, or clean up without relying on reactive tracking."
                .into(),
            ),
          );
        }
      }
      PathologyKind::EmptyWatchSources => {
        if !scope.reads.is_empty() || !scope_coverage_complete(scope) {
          return;
        }
        let (message, help) = absence_finding(
          "`watch` sources do not read any reactive dependency",
          "`watch` sources do not read any known reactive dependency",
          &scope.uncertain_accesses,
          "Pass a ref, a getter, or an array of sources that read reactive state.",
        );
        context.report(self.meta(), scope.span, message, help);
      }
    }
  }
}

fn primary_writes_by_canonical_target<'a>(
  writes: impl Iterator<Item = &'a vue_vet_core::ReactiveWriteFact>,
  bindings: &'a [vue_vet_core::ReactiveBindingFact],
) -> BTreeMap<(usize, String, Option<String>), (String, vue_vet_core::SourceSpan)> {
  let mut primary =
    BTreeMap::<(usize, String, Option<String>), (String, vue_vet_core::SourceSpan)>::new();
  for write in writes {
    let (identity, root, property) = canonical_write_identity(bindings, write);
    let key = (identity, root.to_string(), property.map(str::to_string));
    let path = write_path(write).to_string();
    primary
      .entry(key)
      .and_modify(|(_, span)| {
        if write.span < *span {
          *span = write.span;
        }
      })
      .or_insert((path, write.span));
  }
  primary
}

const fn scope_coverage_complete(scope: &TrackingScopeFact) -> bool {
  scope.unknown_calls.is_empty() && scope.uncertain_accesses.is_empty() && !scope.follow_truncated
}

fn prefer_watch_help(scope: &TrackingScopeFact, block: &ScriptBlockFacts, path: &str) -> String {
  let call = block.calls.iter().find(|call| call.span == scope.span);
  if call.is_some_and(|call| call.flush_option_unresolved) {
    return format!(
      "Use `watch(() => {path}, …)` and keep the current flush and first-run timing."
    );
  }
  let flush = call.and_then(|call| call.flush_option.as_deref());
  match (scope.kind, flush) {
    (TrackingScopeKind::WatchSyncEffect, _) | (TrackingScopeKind::WatchEffect, Some("sync")) => {
      format!(
        "Use `watch(() => {path}, …, {{ immediate: true, flush: 'sync' }})` to preserve the first run and sync flush."
      )
    }
    (TrackingScopeKind::WatchPostEffect, _) | (TrackingScopeKind::WatchEffect, Some("post")) => {
      format!(
        "Use `watch(() => {path}, …, {{ immediate: true, flush: 'post' }})` if a synchronous first run is acceptable; keep `watchPostEffect` when the first run must stay after the current tick."
      )
    }
    (TrackingScopeKind::WatchEffect, Some("pre") | None) => format!(
      "Use `watch(() => {path}, …, {{ immediate: true }})` to preserve `watchEffect`'s first run (flush `'pre'`)."
    ),
    _ => format!("Use `watch(() => {path}, …)` and keep the current flush and first-run timing."),
  }
}

/// Absence-of-evidence findings: prefer hard edges; if only soft evidence remains, mark `(maybe)`.
fn absence_finding(
  confident: &str,
  maybe_prefix: &str,
  uncertain: &[String],
  confident_help: &str,
) -> (String, Option<String>) {
  if uncertain.is_empty() {
    return (confident.into(), Some(confident_help.into()));
  }
  let maybe = uncertain.iter().map(|name| format!("`{name}`")).collect::<Vec<_>>().join(", ");
  (
    format!("{maybe_prefix} (maybe: {maybe})"),
    Some(
      "Analyzed reactivity-shaped accesses (`.value` / `unref` / `toValue` / bare sources) that could not be classified as known bindings. Prefer proven reactive sources; treat this as under-approx uncertainty, not a proven absence.".into(),
    ),
  )
}

macro_rules! pathology_rule {
  ($static_name:ident, $id:literal, $doc:literal, $kind:ident, $scope:ident, $effect_family:expr) => {
    pathology_rule!(
      $static_name,
      $id,
      $doc,
      $kind,
      $scope,
      $effect_family,
      "reactivity",
      Severity::Warning
    );
  };
  (
    $static_name:ident,
    $id:literal,
    $doc:literal,
    $kind:ident,
    $scope:ident,
    $effect_family:expr,
    $category:expr,
    $severity:expr
  ) => {
    static $static_name: PathologyRule = PathologyRule {
      meta: &RuleMeta {
        id: $id,
        category: $category,
        default_severity: $severity,
        confidence: Confidence::High,
        documentation: $doc,
      },
      kind: PathologyKind::$kind,
      scope_filter: TrackingScopeKind::$scope,
      effect_family_only: $effect_family,
    };
  };
}

pathology_rule!(
  P_NO_COMPUTED_SELF_TRIGGER,
  "vue-vet/reactivity/no-computed-self-trigger",
  "rules/reactivity/no-computed-self-trigger",
  SelfTrigger,
  Computed,
  false
);
pathology_rule!(
  P_NO_SIDE_EFFECTS_IN_COMPUTED,
  "vue-vet/reactivity/no-side-effects-in-computed",
  "rules/reactivity/no-side-effects-in-computed",
  SideEffectsInComputed,
  Computed,
  false
);
pathology_rule!(
  P_NO_EFFECT_WRITE_WITHOUT_READ,
  "vue-vet/reactivity/no-effect-write-without-read",
  "rules/reactivity/no-effect-write-without-read",
  WriteWithoutRead,
  WatchEffect,
  true
);
pathology_rule!(
  P_NO_COMPUTED_WITHOUT_DEPENDENCY,
  "vue-vet/reactivity/no-computed-without-dependency",
  "rules/reactivity/no-computed-without-dependency",
  NoDependency,
  Computed,
  false,
  PRACTICE_CATEGORY,
  Severity::Info
);
pathology_rule!(
  P_PREFER_WATCH_OVER_EFFECT_FOR_SINGLE_SOURCE,
  "vue-vet/reactivity/prefer-watch-over-effect-for-single-source",
  "rules/reactivity/prefer-watch-over-effect-for-single-source",
  PreferWatchSingle,
  WatchEffect,
  true,
  PRACTICE_CATEGORY,
  Severity::Info
);
pathology_rule!(
  P_NO_ASSIGNMENT_ONLY_EFFECT_WITH_CONDITIONAL_READ,
  "vue-vet/reactivity/no-assignment-only-effect-with-conditional-read",
  "rules/reactivity/no-assignment-only-effect-with-conditional-read",
  AssignOnlyConditional,
  WatchEffect,
  true
);
pathology_rule!(
  P_NO_ON_SCOPE_DISPOSE_REACTIVE_READ,
  "vue-vet/reactivity/no-on-scope-dispose-reactive-read",
  "rules/reactivity/no-on-scope-dispose-reactive-read",
  DisposeRead,
  OnScopeDispose,
  false
);
pathology_rule!(
  P_NO_EMPTY_WATCH_SOURCES,
  "vue-vet/reactivity/no-empty-watch-sources",
  "rules/reactivity/no-empty-watch-sources",
  EmptyWatchSources,
  WatchSources,
  false
);

struct WatchCallbackTrackingRule;

impl Rule for WatchCallbackTrackingRule {
  fn meta(&self) -> &'static RuleMeta {
    &RuleMeta {
      id: "vue-vet/reactivity/no-watch-callback-as-tracking-scope",
      category: "reactivity",
      default_severity: Severity::Warning,
      confidence: Confidence::High,
      documentation: "rules/reactivity/no-watch-callback-as-tracking-scope",
    }
  }

  fn run_once(&self, context: &mut RuleContext<'_>) {
    for block in &context.script().blocks {
      let scopes = &block.reactivity_graph.scopes;
      let mut index = 0;
      while index + 1 < scopes.len() {
        let Some(sources) = scopes.get(index) else {
          break;
        };
        let Some(callback) = scopes.get(index + 1) else {
          break;
        };
        if sources.kind == TrackingScopeKind::WatchSources
          && callback.kind == TrackingScopeKind::WatchCallback
          && sources.reads.is_empty()
          && !callback.reads.is_empty()
          && scope_coverage_complete(sources)
        {
          let (message, help) = absence_finding(
            "reactive reads in a `watch` callback are not tracked for invalidation",
            "reactive reads in a `watch` callback may not be tracked for invalidation",
            &sources.uncertain_accesses,
            "List dependencies in the `watch` source argument; the callback is a side-effect sink.",
          );
          context.report(self.meta(), callback.span, message, help);
        }
        index += 1;
      }
    }
  }
}

static P_WATCH_CALLBACK_TRACKING: WatchCallbackTrackingRule = WatchCallbackTrackingRule;

struct DestructureSourceRule {
  meta: &'static RuleMeta,
  source: &'static str,
  message: &'static str,
  help: &'static str,
  pinia_store: bool,
}

impl Rule for DestructureSourceRule {
  fn meta(&self) -> &'static RuleMeta {
    self.meta
  }

  fn fact_kinds(&self) -> FactKinds {
    FactKinds::SCRIPT_DESTRUCTURE
  }

  fn run_on(&self, fact: FactRef<'_>, context: &mut RuleContext<'_>) {
    let FactRef::ScriptDestructure { destructure, .. } = fact else {
      return;
    };
    let matched = if self.pinia_store {
      let name = destructure.source_call.as_str();
      name.starts_with("use") && name.ends_with("Store")
    } else {
      destructure.source_call == self.source
    };
    if !matched {
      return;
    }
    context.report(self.meta(), destructure.span, self.message.into(), Some(self.help.into()));
  }
}

macro_rules! destructure_rule {
  ($static_name:ident, $id:literal, $doc:literal, $source:literal, $message:literal, $help:literal, $pinia:expr) => {
    destructure_rule!(
      $static_name,
      $id,
      $doc,
      $source,
      $message,
      $help,
      $pinia,
      "reactivity",
      Severity::Warning
    );
  };
  (
    $static_name:ident,
    $id:literal,
    $doc:literal,
    $source:literal,
    $message:literal,
    $help:literal,
    $pinia:expr,
    $category:expr,
    $severity:expr
  ) => {
    static $static_name: DestructureSourceRule = DestructureSourceRule {
      meta: &RuleMeta {
        id: $id,
        category: $category,
        default_severity: $severity,
        confidence: Confidence::High,
        documentation: $doc,
      },
      source: $source,
      message: $message,
      help: $help,
      pinia_store: $pinia,
    };
  };
}

destructure_rule!(
  I_REACTIVE_DESTRUCTURE,
  "vue-vet/reactivity/no-reactive-destructure",
  "rules/reactivity/no-reactive-destructure",
  "reactive",
  "destructuring `reactive()` loses reactivity for the pulled fields",
  "Keep the reactive object, or use `toRefs` / `toRef`.",
  false
);
destructure_rule!(
  I_SHALLOW_REACTIVE_DESTRUCTURE,
  "vue-vet/reactivity/no-shallow-reactive-destructure",
  "rules/reactivity/no-shallow-reactive-destructure",
  "shallowReactive",
  "destructuring `shallowReactive()` loses reactivity for the pulled fields",
  "Keep the shallowReactive object, or use `toRefs` / `toRef`.",
  false
);
destructure_rule!(
  I_PREFER_STORE_TO_REFS,
  "vue-vet/reactivity/prefer-store-to-refs",
  "rules/reactivity/prefer-store-to-refs",
  "",
  "destructuring a Pinia store loses reactivity for state fields",
  "Use `storeToRefs(store)` for state/getters, and keep actions on the store object.",
  true
);
destructure_rule!(
  I_ROUTE_DESTRUCTURE,
  "vue-vet/reactivity/no-route-destructure",
  "rules/reactivity/no-route-destructure",
  "useRoute",
  "destructuring `useRoute()` snapshots route fields",
  "Keep the route object or read `route.params` / `route.query` through it when later navigation must stay live. One-shot initialization reads can keep the snapshot.",
  false,
  PRACTICE_CATEGORY,
  Severity::Info
);
destructure_rule!(
  I_ROUTER_DESTRUCTURE,
  "vue-vet/reactivity/no-router-destructure",
  "rules/reactivity/no-router-destructure",
  "useRouter",
  "destructuring `useRouter()` is usually unnecessary and can hide API misuse",
  "Keep the router instance and call methods on it.",
  false
);

struct RefOperandRule {
  meta: &'static RuleMeta,
  kinds: &'static [ReactiveBindingKind],
  label: &'static str,
}

impl Rule for RefOperandRule {
  fn meta(&self) -> &'static RuleMeta {
    self.meta
  }

  fn fact_kinds(&self) -> FactKinds {
    FactKinds::SCRIPT_OPERAND
  }

  fn run_on(&self, fact: FactRef<'_>, context: &mut RuleContext<'_>) {
    let FactRef::ScriptOperand { block_kind, operand } = fact else {
      return;
    };
    let Some(block) = script_block(context.script(), block_kind) else {
      return;
    };
    let Some(binding) = reactive_binding_for_operand(block, operand) else {
      return;
    };
    if !self.kinds.contains(&binding.kind) {
      return;
    }
    context.report(
      self.meta(),
      operand.span,
      format!(
        "`{}` is a {} object used as an operand; unwrap with `.value` (or `toValue`)",
        operand.name, self.label
      ),
      Some(format!(
        "Use `{}.value` (or `toValue({})`) when reading the inner value.",
        operand.name, operand.name
      )),
    );
  }
}

macro_rules! ref_operand_rule {
  ($static_name:ident, $id:literal, $doc:literal, $label:literal, [$($kind:ident),+]) => {
    static $static_name: RefOperandRule = RefOperandRule {
      meta: &RuleMeta {
        id: $id,
        category: "reactivity",
        default_severity: Severity::Warning,
        confidence: Confidence::High,
        documentation: $doc,
      },
      kinds: &[$(ReactiveBindingKind::$kind),+],
      label: $label,
    };
  };
}

ref_operand_rule!(
  I_REF_AS_OPERAND,
  "vue-vet/reactivity/no-ref-as-operand",
  "rules/reactivity/no-ref-as-operand",
  "ref",
  [Ref, ShallowRef, CustomRef, ToRef, TemplateRef]
);
ref_operand_rule!(
  I_MODEL_REF_AS_OPERAND,
  "vue-vet/reactivity/no-model-ref-as-operand",
  "rules/reactivity/no-model-ref-as-operand",
  "defineModel ref",
  [ModelRef]
);
ref_operand_rule!(
  I_COMPUTED_AS_OPERAND,
  "vue-vet/reactivity/no-computed-as-operand",
  "rules/reactivity/no-computed-as-operand",
  "computed ref",
  [Computed]
);

struct ReadonlyMutationRule;

impl Rule for ReadonlyMutationRule {
  fn meta(&self) -> &'static RuleMeta {
    &RuleMeta {
      id: "vue-vet/reactivity/no-readonly-mutation",
      category: "reactivity",
      default_severity: Severity::Warning,
      confidence: Confidence::High,
      documentation: "rules/reactivity/no-readonly-mutation",
    }
  }

  fn fact_kinds(&self) -> FactKinds {
    FactKinds::SCRIPT_MEMBER_WRITE
  }

  fn run_on(&self, fact: FactRef<'_>, context: &mut RuleContext<'_>) {
    let FactRef::ScriptMemberWrite { block_kind, write } = fact else {
      return;
    };
    let Some(block) = script_block(context.script(), block_kind) else {
      return;
    };
    let Some(binding) = reactive_binding(block, &write.object) else {
      return;
    };
    if !is_readonly_kind(binding.kind) {
      return;
    }
    context.report(
      self.meta(),
      write.span,
      format!("`{}` is readonly and should not be mutated", write.object),
      Some("Mutate the source reactive state instead of a readonly projection.".into()),
    );
  }
}

static I_READONLY_MUTATION: ReadonlyMutationRule = ReadonlyMutationRule;

#[must_use]
pub fn matrix_rules() -> Vec<&'static dyn Rule> {
  vec![
    &B_NO_AFTER_AWAIT_DEPENDENCY_IN_COMPUTED,
    &B_NO_OUTSIDE_TRACKING_DEPENDENCY_IN_COMPUTED,
    &B_NO_AFTER_AWAIT_DEPENDENCY_IN_WATCH_SOURCES,
    &B_NO_OUTSIDE_TRACKING_DEPENDENCY_IN_WATCH_SOURCES,
    &B_NO_AFTER_AWAIT_DEPENDENCY_IN_EFFECT_SCOPE,
    &B_NO_OUTSIDE_TRACKING_DEPENDENCY_IN_EFFECT_SCOPE,
    &A_DEFINE_EXPOSE,
    &P_NO_COMPUTED_SELF_TRIGGER,
    &P_NO_SIDE_EFFECTS_IN_COMPUTED,
    &P_NO_EFFECT_WRITE_WITHOUT_READ,
    &P_NO_COMPUTED_WITHOUT_DEPENDENCY,
    &P_PREFER_WATCH_OVER_EFFECT_FOR_SINGLE_SOURCE,
    &P_NO_ASSIGNMENT_ONLY_EFFECT_WITH_CONDITIONAL_READ,
    &P_NO_ON_SCOPE_DISPOSE_REACTIVE_READ,
    &P_NO_EMPTY_WATCH_SOURCES,
    &P_WATCH_CALLBACK_TRACKING,
    &I_REACTIVE_DESTRUCTURE,
    &I_SHALLOW_REACTIVE_DESTRUCTURE,
    &I_PREFER_STORE_TO_REFS,
    &I_ROUTE_DESTRUCTURE,
    &I_ROUTER_DESTRUCTURE,
    &I_REF_AS_OPERAND,
    &I_MODEL_REF_AS_OPERAND,
    &I_COMPUTED_AS_OPERAND,
    &I_READONLY_MUTATION,
  ]
}
