...
use super::*;
use crate::source_contracts::{SourceContractStats, collect_source_contract_facts_with_stats};
...
fn contract_stats(source: &str) -> (vue_vet_core::SourceContractFacts, u64) {
  let (facts, stats) = contract_stats_full(source);
  (facts, stats.work())
}

fn contract_stats_full(source: &str) -> (vue_vet_core::SourceContractFacts, SourceContractStats) {
...
fn value_contracts_shared_bag_and_destructure_growth_stays_subquadratic() {
  ...
  for size in [16_u64, 32, 64] {
    ...
    let (contracts, stats) = contract_stats_full(&source);
    let work = stats.work();
    let expected = usize::try_from(size).unwrap_or(usize::MAX);
