...
use vue_vet_core::{FileId, FindingExplain, RuleExplain, ScopeExplain, WorkspaceRoot};
...
use crate::{
  ...
  types::{AnalysisSnapshot, ChangeSet, SessionError, SessionOptions},
};
...
impl ProjectSession {
  ...
  /// Turn on `ignore_test` for this session. CLI `--ignore-test` uses this.
  pub const fn enable_ignore_test(&mut self) {
    self.config.ignore_test = true;
  }
