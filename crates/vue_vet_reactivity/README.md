...
(`REACTIVITY_GRAPH_VERSION = 13` in `vue_vet_core`: imported
`const api = createApi()` exports as ValueBag; v12 options-object callback
slots follow `export*` / re-export barrels; v11 options-object callback
Ref bags (`setup({ values })`), ComponentFactory props seeds,
