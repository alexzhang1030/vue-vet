...
fn named_namespace_string_type_only_bare_and_shadow_apis_keep_summary_and_graph() {
  ...
  let (named_graph, _, named_scan, named_usage) = graph_and_summary(named, ScriptKind::Script);
  ...
  let (string_graph, _, string_scan, string_usage) =
...
fn populated_local_composable_bags_factories_and_forwarders_keep_usage_walk() {
  ...
  let (bag_graph, _, bag_scan, bag_usage) = graph_and_summary(bag, ScriptKind::Setup);
...
state.value.count = 2
";
  let (gated, _, scan, usage) = graph_and_summary_with_seeds(local, ScriptKind::Setup, &seeds);
...
";
  ...
  let (seed_graph, _, seed_scan, seed_usage) =
