...
const DERIVATION_PRACTICE_RULES: &[&str] =
  &["prefer-sync-ref-one-way", "prefer-conditional-watch-source"];

#[test]
#[expect(clippy::panic, reason = "fixture IO must fail the golden test")]
fn derivation_practice_rule_fixtures() {
  let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
  for rule in DERIVATION_PRACTICE_RULES {
      ...
      std::fs::read_dir(&invalid_dir).unwrap_or_else(|error| panic!("{invalid_dir:?}: {error}"))
    {
      ...
      if std::env::var_os("UPDATE_DERIVATION_PRACTICE_SNAPSHOTS").is_some() {
      ...
      assert!(
        parsed.iter().filter(|diagnostic| diagnostic.rule_id.ends_with(*rule)).all(|diagnostic| {
          diagnostic.category == PRACTICE_CATEGORY
            && diagnostic.severity == Severity::Info
            && !diagnostic.affects_score()
            && !diagnostic.affects_exit()
        }),
        "{logical} must stay on the practice channel"
      );
...
const SCHEDULING_PRACTICE_RULES: &[&str] =
  &["prefer-queued-watch-flush", "prefer-attached-effect-scope", "prefer-lazy-computed-async"];

#[test]
#[expect(clippy::panic, reason = "fixture IO must fail the golden test")]
fn scheduling_practice_rule_fixtures() {
  let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
  for rule in SCHEDULING_PRACTICE_RULES {
    let invalid_dir = root.join(format!("fixtures/rules/{rule}/invalid"));
    let valid_dir = root.join(format!("fixtures/rules/{rule}/valid"));
    let snap_dir = root.join(format!("fixtures/snapshots/{rule}"));
    std::fs::create_dir_all(&snap_dir)
      ...
      std::fs::read_dir(&invalid_dir).unwrap_or_else(|error| panic!("{invalid_dir:?}: {error}"))
    {
      ...
      if std::env::var_os("UPDATE_SCHEDULING_PRACTICE_SNAPSHOTS").is_some() {
        std::fs::write(&snap_path, format!("{actual}\n"))
          .unwrap_or_else(|error| panic!("write {snap_path:?}: {error}"));
      }
      let expected = std::fs::read_to_string(&snap_path)
