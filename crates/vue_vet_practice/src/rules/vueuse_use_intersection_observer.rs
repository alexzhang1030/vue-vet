...
const RECIPE: PracticeRecipe = PracticeRecipe {
  rule_id: "vue-vet/practice/vueuse-use-intersection-observer",
  documentation: "rules/practice/vueuse-use-intersection-observer",
  confidence: Confidence::Medium,
  min_vue: None,
  recommend: EcosystemApi {
    package: "@vueuse/core",
    export: "useIntersectionObserver",
    docs_url: "https://vueuse.org/core/useIntersectionObserver/",
    import_example: "import { useIntersectionObserver } from '@vueuse/core'",
  },
};
...
pub(super) struct VueuseUseIntersectionObserver;

pub(super) static RULE: VueuseUseIntersectionObserver = VueuseUseIntersectionObserver;

impl Rule for VueuseUseIntersectionObserver {
  ...
  fn run_once(&self, context: &mut RuleContext<'_>) {
    ...
    for (span, help) in findings {
      context.report_with_recommendation(
        self.meta(),
        span,
        "This constructs an `IntersectionObserver` inside a setup lifecycle hook without `disconnect`; consider VueUse `useIntersectionObserver` for automatic cleanup.".into(),
        Some(help),
...
mod tests {
  ...
  #[test]
  fn reports_lifecycle_intersection_observer_without_disconnect() {
    let diagnostics = run(vec![call("onMounted", 0), call("IntersectionObserver", 20)]);
  ...
  fn stays_quiet_without_lifecycle_hook() {
    let diagnostics = run(vec![call("IntersectionObserver", 0)]);
    assert!(diagnostics.is_empty());
  }
}
