...
const RECIPE: PracticeRecipe = PracticeRecipe {
  rule_id: "vue-vet/practice/vueuse-use-resize-observer",
  documentation: "rules/practice/vueuse-use-resize-observer",
  confidence: Confidence::Medium,
  min_vue: None,
  recommend: EcosystemApi {
    package: "@vueuse/core",
    export: "useResizeObserver",
    docs_url: "https://vueuse.org/core/useResizeObserver/",
    import_example: "import { useResizeObserver } from '@vueuse/core'",
  },
};
...
pub(super) struct VueuseUseResizeObserver;

pub(super) static RULE: VueuseUseResizeObserver = VueuseUseResizeObserver;

impl Rule for VueuseUseResizeObserver {
  ...
  fn run_once(&self, context: &mut RuleContext<'_>) {
    ...
    let findings = context
      ...
      .filter_map(|block| {
        observer_ctor_without_disconnect(block, "ResizeObserver").map(|call| {
    ...
    for (span, help) in findings {
      context.report_with_recommendation(
        self.meta(),
        span,
        "This constructs a `ResizeObserver` inside a setup lifecycle hook without `disconnect`; consider VueUse `useResizeObserver` for automatic cleanup.".into(),
        Some(help),
...
mod tests {
  ...
  fn run(calls: Vec<ScriptCallFact>) -> Vec<vue_vet_core::Diagnostic> {
    ...
    practice_registry().run(Path::new("src/Ro.vue"), "", &TemplateFacts::default(), &script)
  }

  #[test]
  fn reports_lifecycle_resize_observer_without_disconnect() {
    let diagnostics = run(vec![call("onMounted", 0), call("ResizeObserver", 20)]);
  ...
  fn stays_quiet_when_disconnect_present() {
    let diagnostics =
      run(vec![call("onMounted", 0), call("ResizeObserver", 20), call("disconnect", 40)]);
    assert!(diagnostics.is_empty());
  }

  #[test]
  fn stays_quiet_without_lifecycle_hook() {
    let diagnostics = run(vec![call("ResizeObserver", 0)]);
    assert!(diagnostics.is_empty());
  }
}
