...
Vue 3.5 `<script setup>` restores instance context across top-level `await` (`withAsyncContext`). Registering `onServerPrefetch` after `await` still binds to this instance.
...
- `fixtures/rules/no-on-server-prefetch-after-await/valid/safe.vue`
