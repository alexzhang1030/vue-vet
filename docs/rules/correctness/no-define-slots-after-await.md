...
`defineSlots` is a compiler macro hoisted by `compileScript`. Source position after top-level `await` is not a runtime defect.
...
- `fixtures/rules/no-define-slots-after-await/valid/safe.vue`
