...
Vue 3.5 `<script setup>` restores instance context across top-level `await` (`withAsyncContext`). Registering `onBeforeUnmount` after `await` still binds to this instance.
...
- `fixtures/rules/no-on-before-unmount-after-await/valid/safe.vue`
