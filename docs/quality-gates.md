...
| `fixtures/projects/practice-vueuse` | vue, precision | Practice channel: VueUse cleanup recipes (timeout/interval/raf/listeners/observers/debounce) + getter-evidence `prefer-to-value` (numeric `unref` stays quiet). |
...
| `fixtures/projects/reactivity-rules` | vue, reactivity, precision | Unused binding / prefer-computed TPs; SafePatterns FP pins. Dynamic-dep fixtures stay quiet. |
...
| Diff | Full analyze (unmeasured) then `filter_diff` with a fixed changed-file set (no git). Cache teardown is outside the measured closure. |
